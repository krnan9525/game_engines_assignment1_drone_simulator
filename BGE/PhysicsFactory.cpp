...
/*shared_ptr<Friction_Controller> BGE::PhysicsFactory::CreateBox2(float width, float height, float depth, glm::vec3 pos, glm::quat quat, bool kinematic, bool attachToGame)
{
	...
	shared_ptr<Friction_Controller> boxController = make_shared<Friction_Controller>(boxShape, body, boxMotionState, 10.0f);
	boxController->Initialise();
...
shared_ptr<PhysicsController> PhysicsFactory::CreateBox(float width, float height, float depth, glm::vec3 pos, glm::quat quat, bool kinematic, bool attachToGame)
{
	// Create the shape
	btCollisionShape * boxShape = new btBoxShape(btVector3(width, height, depth) * 0.50);
	btScalar mass = (kinematic) ? 0 : 10.0f;
	btVector3 boxInertia(0, 0, 0);
	boxShape->calculateLocalInertia(mass, boxInertia);
	...
	boxController->tag = "box_controller";
	body->setUserPointer(boxController.get());
	//body->setCollisionFlags(body->getCollisionFlags() | btCollisionObject::CF_CUSTOM_MATERIAL_CALLBACK);
	box->Attach(boxController);
	return boxController;
...
shared_ptr<PhysicsController> PhysicsFactory::CreateCylinder(float radius, float height, glm::vec3 pos, glm::quat quat, bool kinematic, bool attachToGame)
{
	...
	btScalar mass = (kinematic) ? 0 : 0.25f;
...
shared_ptr<Friction_Controller> PhysicsFactory::CreateCylinder2(float radius, float height, glm::vec3 pos, glm::quat quat, string tagg, bool kinematic, bool attachToGame)
{
	// Create the shape
	btCollisionShape * shape = new btCylinderShape(btVector3(radius, height * 0.5f, radius));
	...
	// Create the physics component and add it to the box
	shared_ptr<Friction_Controller> component = make_shared<Friction_Controller>(shape, body, motionState);
	body->setUserPointer(component.get());
	cyl->Attach(component);
	component->tag = tagg;
	return component;
